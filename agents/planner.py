...
class PlannerAgent:
    ...
    def plan(self, profile: Dict[str, Any], user_goal: str = "General EDA", max_items: int = 8) -> Dict[str, Any]:
        """
    ...
    def _build_planner_prompt(self, profile: Dict[str, Any], user_goal: str, max_items: int) -> str:
...
"""
    ...
    def _create_fallback_plan(self, profile: Dict[str, Any], max_items: int) -> Dict[str, Any]:
        ...
        categorical_cols = [col['name'] for col in columns if not col.get('numeric') and col.get('top_values')]
        if categorical_cols:
